...
def check_places(data):
    r = requests.get(settings.PATH_PLA)
    places = r.json()
    for place in places:
...
def MeasurementsCreate(request):
    if request.method == 'POST':
        ...
        for measurement in data_json:
                    if check_variable(measurement) == True and check_places(measurement) == True:
                        ...
                        return HttpResponse("unsuccessfully created measurement. Variable does not exist", status=400)
